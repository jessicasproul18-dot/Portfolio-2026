...
/**
 * **CURSOR INFO**
 * SECTION TYPE: Website development project gallery with multi-image lightbox
 * BEST FOR: Portfolio website development pages
 * VISUAL STYLE: Modern, minimal, and clean
 * LAYOUT: Portrait headshot grid; lightbox slideshow with thumbnail strip
 * CONTENT ELEMENTS: Project cards with labels, fullscreen slideshow, upcoming image thumbs
 * CONVERSION ROLE: Showcase web builds by project
 * IDEAL POSITION: Main content under page header on website development page
 * NOTES / MODIFIERS: Portrait (3/4) covers with object-top; groups multi-page sites
 */
...
import {
  ...
  type WebsiteDevelopmentProject,
} from '@/lib/websiteDevelopment';

export function WebsiteDevelopmentGallerySection() {
  const [activeProject, setActiveProject] = useState<WebsiteDevelopmentProject | null>(null);
  const [slideIndex, setSlideIndex] = useState(0);
  const titleId = useId();

  const handleOpen = (project: WebsiteDevelopmentProject) => {
            ...
            {websiteDevelopmentProjects.map((project) => {
              ...
              return (
                <li key={project.id}>
                  ...
                  >
                    <div className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-zinc-100 shadow-sm">
                      <Image
                        src={cover.src}
                        alt={cover.alt}
                        fill
                  ...
                  return (
                    <li key={`${image.src}-${index}`} className="shrink-0">
                      <button
                        ...
                        className={[
                          'relative block h-20 w-14 overflow-hidden rounded-lg border transition md:h-24 md:w-16',
                          isActive
